...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        newDist = {}
        for p in self.allPositions:
            newDist[p] = self.getPositionDistribution(gameState, p)
        newBeliefs = DiscreteDistribution()
        for p in self.allPositions:
            sum = 0
            for p1 in self.allPositions:
                sum += newDist[p1][p] * self.beliefs[p1]
            newBeliefs[p] = sum
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % n])
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            newPart.append(self.getPositionDistribution(gameState, pos).sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        jointPos = itertools.product(self.legalPositions, repeat=self.numGhosts)
        jointPos = [pos for pos in jointPos]
        n = len(jointPos)
        random.shuffle(jointPos)
        for i in range(self.numParticles):
            self.particles.append(jointPos[i % n])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.particles:
            if pos in beliefs.keys():
                beliefs[pos] += 1
            else:
                beliefs[pos] = 1
        for pos in beliefs.keys():
            for ghost in range(self.numGhosts):
                beliefs[pos] *= self.getObservationProb(observation[ghost], gameState.getPacmanPosition(),
                    pos[ghost], self.getJailPosition(ghost))
        if beliefs.total():
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(beliefs.sample())
