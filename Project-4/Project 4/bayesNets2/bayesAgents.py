...
def constructBayesNet(gameState):
    ...
    for var1 in [X_POS_VAR, Y_POS_VAR]:
        for var2 in HOUSE_VARS:
            edges.append((var1,  var2))
            
    for var1 in HOUSE_VARS:
        for var2 in obsVars:
            edges.append((var1, var2))
...
def fillYCPT(bayesNet, gameState):
    ...
    """
    """from layout import PROB_BOTH_TOP, PROB_BOTH_BOTTOM, PR"""
...
def fillObsCPT(bayesNet, gameState):
    ...
    house_vals = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    for i in range(0,4):
        housePos = gameState.getPossibleHouses()[i]
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                foodHouse = assignment[FOOD_HOUSE_VAR] == house_vals[i]
                ghostHouse = assignment[GHOST_HOUSE_VAR] == house_vals[i]
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    probTable = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    maxProb = 0
    for val in HOUSE_VALS:
        evidence[FOOD_HOUSE_VAR] = val
        if probTable.getProbability(evidence) > maxProb:
            maxProb = probTable.getProbability(evidence)
            maxVal = val
    return {FOOD_HOUSE_VAR: maxVal}
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probTable = inference.inferenceByVariableElimination(self.bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
        evidence[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        probLeft = probTable.getProbability(evidence)
        evidence[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        probRight = probTable.getProbability(evidence)
        leftExpectedValue = probLeft * WON_GAME_REWARD + probRight * GHOST_COLLISION_REWARD
        rightExpectedValue = probRight * WON_GAME_REWARD + probLeft * GHOST_COLLISION_REWARD
